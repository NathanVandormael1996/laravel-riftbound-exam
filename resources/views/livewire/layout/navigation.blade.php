...
<nav x-data="{ open: false }" class="bg-sentry-darker border-b border-sentry-border sticky top-0 z-50">
        ...
        <div class="flex justify-between h-20">
            <div class="flex items-center">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @auth
                        <a href="{{ route('dashboard') }}" wire:navigate class="sentry-label hover:text-white transition-colors {{ request()->routeIs('dashboard') ? 'text-white border-b-2 border-sentry-light pb-1' : '' }}">
                            Dashboard
                        </a>
                        @if(auth()->user()->role === \App\Enums\UserRole::ADMIN)
                            <a href="{{ route('admin.dashboard') }}" wire:navigate class="sentry-label hover:text-sentry-light transition-colors flex items-center gap-1 {{ request()->routeIs('admin.*') ? 'text-sentry-light border-b-2 border-sentry-light pb-1' : '' }}">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6 space-x-4">
                ...
                @auth
                    <div class="flex items-center space-x-4">
                        <div class="text-right">
                            ...
                            <div class="text-xs text-sentry-light opacity-70 uppercase tracking-widest">{{ auth()->user()->role }}</div>
                        </div>
                ...
                @else
                    <a href="{{ route('login') }}" wire:navigate class="btn-sentry-white px-6 py-2 text-xs">
                        Login
                    </a>
                    <a href="{{ route('register') }}" wire:navigate class="btn-sentry-primary px-6 py-2 text-xs">
                        Register
                    </a>
                @endauth
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-sentry-darker border-t border-sentry-border">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <a href="{{ route('cart.index') }}" wire:navigate class="block px-4 py-3 sentry-label hover:bg-sentry-deep flex justify-between items-center">
                <span>Basket</span>
                @if($this->cartCount > 0)
                    <span class="bg-sentry-light text-sentry-deep px-2 py-0.5 rounded-full text-[10px] font-bold">{{ $this->cartCount }}</span>
                @endif
            </a>
            @auth
                <a href="{{ route('dashboard') }}" wire:navigate class="block px-4 py-3 sentry-label hover:bg-sentry-deep">Dashboard</a>
                @if(auth()->user()->role === \App\Enums\UserRole::ADMIN)
                    <a href="{{ route('admin.dashboard') }}" wire:navigate class="block px-4 py-3 sentry-label text-sentry-light hover:bg-sentry-deep">⚙ Command Center</a>
                @endif
            @endauth
        </div>
        ...
        <div class="pt-4 pb-1 border-t border-sentry-border">
            @auth
                <div class="px-4 py-3">
                    ...
                    <div class="text-sm text-sentry-light">{{ auth()->user()->email }}</div>
                </div>
            ...
            @else
                <div class="p-4 space-y-2">
                    <a href="{{ route('login') }}" wire:navigate class="block w-full text-center btn-sentry-white py-3">Login</a>
                    <a href="{{ route('register') }}" wire:navigate class="block w-full text-center btn-sentry-primary py-3">Register</a>
                </div>
            @endauth
        </div>
    </div>

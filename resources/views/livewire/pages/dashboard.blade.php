        ...
        <div class="flex flex-col md:flex-row gap-8">
            ...
            <aside class="w-full md:w-80 shrink-0 space-y-8">
                <div class="sentry-card p-8 text-center relative overflow-hidden group">
                    ...
                    <div class="relative z-10 space-y-4">
                        ...
                        <div>
                            ...
                            <p class="sentry-label opacity-60">{{ auth()->user()->role->value }}</p>
                        </div>
                        <div class="pt-4 flex justify-center space-x-4">
                            <div class="text-center">
                                <div class="text-xl font-mono font-bold text-sentry-light">{{ auth()->user()->orders()->count() }}</div>
                ...
                <nav class="sentry-glass p-2 space-y-1">
                    <a href="{{ route('dashboard') }}" wire:navigate class="flex items-center space-x-3 px-4 py-3 rounded-lg {{ request()->routeIs('dashboard') ? 'bg-sentry-purple/10 text-sentry-light border-l-2 border-sentry-purple' : 'hover:bg-sentry-deep text-sentry-muted hover:text-sentry-light' }} transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path></svg>
                        <span class="font-medium">Overview</span>
                    </a>
                    <a href="{{ route('orders.index') }}" wire:navigate class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-sentry-deep text-sentry-muted hover:text-sentry-light transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
                        <span class="font-medium">My Orders</span>
                    </a>
                    <a href="{{ route('profile') }}" wire:navigate class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-sentry-deep text-sentry-muted hover:text-sentry-light transition-all">
                                ...
                                <div class="space-y-3 pt-2">
                                    @foreach($this->recentOrders as $order)
                                        <div class="flex justify-between items-center p-3 rounded bg-sentry-darker/50 border border-sentry-border/30">
                                            ...
                                            <div class="text-xs uppercase tracking-widest font-bold {{ $order->status->value === 'completed' ? 'text-green-400' : 'text-sentry-coral' }}">
                                                {{ $order->status->value }}
                                            </div>
                                        </div>
                                    @endforeach

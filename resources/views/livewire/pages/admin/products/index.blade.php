                ...
                <tbody class="divide-y divide-sentry-border/40">
                    @forelse($this->products as $product)
                        <tr class="group hover:bg-sentry-purple/5 transition-colors duration-150">
                            <td class="px-6 py-4">
                                <div class="font-semibold text-white group-hover:text-sentry-light transition-colors">{{ $product->name }}</div>
                                <div class="font-mono text-[10px] text-sentry-light opacity-30 mt-0.5 tracking-wider">{{ $product->slug }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="font-mono text-[11px] text-sentry-light opacity-60">{{ $product->category->name }}</span>
                            </td>
                            <td class="px-6 py-4 font-mono text-sm font-bold text-sentry-light">€{{ number_format($product->price, 2) }}</td>
                            <td class="px-6 py-4 font-mono text-sm {{ $product->stock < 10 ? 'text-sentry-pink' : 'text-sentry-light opacity-70' }}">
                                {{ $product->stock }}
                                @if($product->stock < 10)
